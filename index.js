...
// ============================================================================
// 🔧 ROTAS DE TAREFAS DIRETAS NO INDEX.JS
// ============================================================================
...
let tarefasMemoria = [
    ...
    { 
        id: 2, 
        titulo: 'Criar rotas', 
        descricao: 'Criar as rotas da aplicação', 
        concluida: true, 
        prioridade: 'media',
        data: '2024-01-02' 
    },
    { 
        ...
        descricao: 'Implementar as funcionalidades principais', 
        concluida: false, 
        prioridade: 'alta',
        data: '2024-01-03' 
    }
...
// ROTA GET /api/tarefas - DIRETO NO INDEX.JS
app.get('/api/tarefas', (req, res) => {
    console.log('✅ Rota GET /api/tarefas funcionando diretamente!');
    
    res.json({
        success: true,
        data: tarefasMemoria,
        total: tarefasMemoria.length,
        timestamp: new Date().toISOString()
    });
});

// ROTA POST /api/tarefas - DIRETO NO INDEX.JS
app.post('/api/tarefas', (req, res) => {
    console.log('✅ Rota POST /api/tarefas funcionando!');
    ...
    tarefasMemoria.push(novaTarefa);
...
// ROTA PUT /api/tarefas/:id - DIRETO NO INDEX.JS
app.put('/api/tarefas/:id', (req, res) => {
    const { id } = req.params;
    console.log(`✅ Rota PUT /api/tarefas/${id} funcionando!`);
    console.log('Dados recebidos:', req.body);
    
    const { titulo, descricao, concluida, prioridade } = req.body;
    
    const tarefaIndex = tarefasMemoria.findIndex(t => t.id == id);
    ...
    tarefasMemoria[tarefaIndex] = {
        ...tarefasMemoria[tarefaIndex],
        titulo: titulo || tarefasMemoria[tarefaIndex].titulo,
        descricao: descricao !== undefined ? descricao : tarefasMemoria[tarefaIndex].descricao,
        concluida: concluida !== undefined ? concluida : tarefasMemoria[tarefaIndex].concluida,
        prioridade: prioridade || tarefasMemoria[tarefaIndex].prioridade || 'media',
    ...
    res.json({
        ...
        data: tarefasMemoria[tarefaIndex],
        timestamp: new Date().toISOString()
    });
});

// ROTA DELETE /api/tarefas/:id - DIRETO NO INDEX.JS
app.delete('/api/tarefas/:id', (req, res) => {
    const { id } = req.params;
    console.log(`✅ Rota DELETE /api/tarefas/${id} funcionando!`);
    ...
    const tarefaRemovida = tarefasMemoria.splice(tarefaIndex, 1)[0];
